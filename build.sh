...
if ! test -f "linux-$KERNEL_VERSION/vmlinux"; then
  ...
  echo "CONFIG_KASAN_OUTLINE=y" >> linux-$KERNEL_VERSION/.config
  echo "CONFIG_TEST_KASAN_MODULE=n" >> linux-$KERNEL_VERSION/.config
  echo "CONFIG_BPF_SYSCALL=y" >> linux-$KERNEL_VERSION/.config
  echo "CONFIG_BPF_JIT=y" >> linux-$KERNEL_VERSION/.config
